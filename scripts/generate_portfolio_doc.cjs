  ...
  const doc = new Document({
    ...
    sections: [{
      ...
      children: [
        ...
        // ETAPA 6
        createHeading2("Etapa 6: Reestructuración de Escuelas, Matrícula Atómica y Stored Procedure (08 de Septiembre, 2026)"),
        createBullet("Objetivo de la Etapa:", "Dar cumplimiento a los requerimientos de la dirección: segregación estricta de escuelas, depuración curricular a 20 cursos, creación de cuentas en la ficha y resolución definitiva de la persistencia en base de datos."),
        createBullet("Commits Registrados:", "64f27bb, dffdee5."),
        ...
        createBullet("2. Depuración Curricular Estricta a 20 Cursos Oficiales:", "Se eliminaron todos los cursos genéricos o que no correspondían a la oferta aprobada, fijando exactamente: 10 Cursos de Oficios y 10 Cursos de Seguridad Privada."),
        createBullet("3. Retiro del Botón de Admisión en el Header y Botón Temarios:", "Se eliminó el botón general de 'Ficha de Admisión' y el botón de temarios. La inscripción se canaliza ahora directamente desde el botón 'Inscribirme' de cada curso en el catálogo, abriendo la ficha con ese curso preseleccionado en la Sección 1."),
        createBullet("4. Creación de Cuenta con Contraseña en la Misma Ficha:", "Se incorporó la Sección 2.1 en `EnrollmentForm.jsx`. El RUT ingresado por el alumno se fija automáticamente como su usuario oficial de acceso y se le solicita definir y confirmar una contraseña secreta."),
        createBullet("5. Procedimiento Almacenado Atómico `process_enrollment_registration`:", "Creación y despliegue del Stored Procedure con privilegios `SECURITY DEFINER` en Supabase PostgreSQL. Orquesta en una única transacción: validación de RUT con Módulo 11, regla de 1 curso activo, creación en `auth.users` y `public.users` (rol STUDENT con Bcrypt), inserción en `public.escuela_oficio` o `public.escuela_seguridad` y sincronización en `public.enrollments` con resolución de foreign keys."),

        // ETAPA 7
        createHeading2("Etapa 7: Detección Geográfica Regional, Modalidades Reactivas y Gestor Administrativo (Septiembre, 2026)"),
        createBullet("Objetivo de la Etapa:", "Personalizar la experiencia del estudiante según su ubicación geográfica, habilitar estado de cursos 'Próximamente' y permitir al administrador controlar las modalidades presencial/virtual."),
        createBullet("Commits Registrados:", "e0809c3, d4502df."),
        createBullet("1. Selectores Dinámicos de Región y Ciudad (Commit e0809c3):", "Integración de la división político-administrativa de Chile completa (16 regiones y sus provincias/ciudades) en la Ficha de Inscripción (`EnrollmentForm.jsx`). Al seleccionar una región, el listado de ciudades se actualiza dinámicamente."),
        createBullet("2. Detección Inteligente de Modalidad por Procedencia (Commit e0809c3):", "Si el estudiante reside en Arica, el sistema habilita la elección entre modalidad Presencial (Sede Central Blanco Encalada N°666) o Aula Virtual. Si reside en cualquier otra ciudad o región de Chile (Iquique, Antofagasta, Santiago, etc.), el sistema asigna y bloquea automáticamente la modalidad en formato 100% Virtual Online sincrónico vía Zoom y plataforma 24/7."),
        createBullet("3. Estado 'Próximamente' para Cursos Futuros (Commit e0809c3):", "Capacidad de marcar cursos que abrirán en futuras fechas sin cupos activos. En el catálogo se presenta el distintivo amarillo 'Próximamente' y el botón de acción cambia a 'Consultar por Próxima Fecha' hacia WhatsApp."),
        createBullet("4. Configuración Reactiva de Modalidades Presencial / Virtual (Commit d4502df):", "Implementación de switches independientes en el Gestor de Cursos (`CourseManagerModal.jsx`) y en la vista de configuración administrativa (`SettingsView.jsx`), permitiendo al Administrador activar/desactivar la etiqueta 'Presencial', 'Virtual' o ambas para cada capacitación. El catálogo (`Services.jsx`) y la ficha de admisión (`EnrollmentForm.jsx`) adaptan reactivamente sus insignias y mensajes explicativos."),
        createBullet("5. Acceso Unificado al Gestor desde Administración del Sitio (Commit d4502df):", "Integración del botón '⚙️ Modificar Disponibilidad, Cupos y Fechas' directamente en el encabezado de `SiteAdminView.jsx` y `SettingsView.jsx`, facilitando el control operativo de toda la oferta académica."),
